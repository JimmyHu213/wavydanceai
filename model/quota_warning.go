...
// send time, so it must be followed by an actual send attempt.
...
func buildQuotaWarningEmail(remaining int64) (subject string, content string) {
	...
	lead := fmt.Sprintf("Your %s balance is running low — you have <strong>%s</strong> remaining.", systemName, quotaDollars(remaining))
	if remaining <= 0 {
		subject = fmt.Sprintf("Your %s balance has run out", config.SystemName)
		title = "Balance exhausted"
		lead = fmt.Sprintf("Your %s balance has run out. Requests will fail until you top up.", systemName)
	}
	topUpLink := fmt.Sprintf("%s/console/topup", config.ServerAddress)
	safeLink := html.EscapeString(topUpLink)
	content = message.EmailTemplate(title, fmt.Sprintf(`
				<p>Hi,</p>
				<p>%s</p>
				<p style="text-align: center; margin: 30px 0;">
					<a href="%s" style="background-color: #2e8fb0; color: #ffffff; padding: 12px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold;">Top up now</a>
				</p>
